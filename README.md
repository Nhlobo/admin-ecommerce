...
This admin app now reads the backend URL from `BACKEND_URL` at runtime through `js/runtime-config.js` generated by `server.js`.

Set it in Render or your host environment:

```
BACKEND_URL=https://your-backend-url.onrender.com
```

> **Important:** Database connection strings and DB credentials must stay in the backend service environment variables, not in this frontend admin app.
